...
class UserContext(BaseModel):
    ...
    - [新增] Knowledge files from knowledge/ directory
    """
...
class ContextLoader:
    ...
    def load_context(self, user: User) -> UserContext:
        ...
        # [新增] Load knowledge directory files
        knowledge = self._load_knowledge_context(workspace_path)
    ...
    def _load_knowledge_context(self, workspace_path: Path) -> str:
        """
        [新增] Load knowledge directory files.
        
        Loads all .md and .txt files from knowledge/ directory
        and combines them into a single context string.
        
        Args:
            workspace_path: User workspace path
        
        Returns:
            Combined knowledge content string
        """
        ...
        # Load all .md and .txt files from knowledge directory
        for file_path in sorted(knowledge_dir.iterdir()):
            if file_path.is_file() and file_path.suffix in (".md", ".txt"):
                try:
                    ...
                    print(f"[Context] Error loading knowledge file {file_path}: {e}")
        
        if knowledge_parts:
            return "# 知识库\n\n以下是用户上传的知识文档，回答问题时请参考这些内容：\n" + "\n---\n".join(knowledge_parts)
